...
TEST_CASE("COW semantics") {
    ...
    SECTION("Read operation do not copy state") {
        const auto v2 = v1;  // NOLINT(performance-unnecessary-copy-initialization)
        REQUIRE(v2.Size() == 1);
        REQUIRE(&v2.Get(0) == p1);
    }

    SECTION("Assignment does not change state") {
        const auto& r = v1;
        v1 = r;
        REQUIRE(v1.Size() == 1);
        REQUIRE(&v1.Get(0) == p1);
...
TEST_CASE("Signature class") {
    STATIC_CHECK(std::is_copy_constructible_v<COWVector>);
    STATIC_CHECK(std::is_copy_constructible_v<COWVector>);
    STATIC_CHECK(std::is_assignable_v<COWVector, const COWVector&>);
    STATIC_CHECK(std::is_assignable_v<COWVector, COWVector&&>);
}