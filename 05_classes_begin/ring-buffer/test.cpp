#include "ring_buffer.h"

#include <algorithm>
#include <catch2/catch_test_macros.hpp>
#include <cstdio>
#include <random>
#include <ranges>

TEST_CASE("Empty") {
    const RingBuffer buffer(1);

    REQUIRE(buffer.Size() == 0);
    REQUIRE(buffer.Empty());
}

TEST_CASE("PushAndPop") {
    RingBuffer buffer(2);

    REQUIRE(buffer.TryPush(0));
    REQUIRE(buffer.TryPush(1));
    REQUIRE_FALSE(buffer.TryPush(2));

    REQUIRE(buffer.Size() == 2);
    REQUIRE_FALSE(buffer.Empty());

    int i = 0;
    REQUIRE(buffer.TryPop(&i));
    REQUIRE(i == 0);
    REQUIRE(buffer.TryPop(&i));
    REQUIRE(i == 1);

    REQUIRE_FALSE(buffer.TryPop(&i));
    REQUIRE(buffer.Size() == 0);
    REQUIRE(buffer.Empty());
}

TEST_CASE("Random!") {
    std::mt19937 gen{485'392'193};  // NOLINT(cert-msc51-cpp,cert-msc51-c, cert-msc32*)
    gen.seed(42);                   // NOLINT(cert-msc51-cpp,cert-msc51-cpp, cert-msc32*)
    std::bernoulli_distribution dist{.6};
    RingBuffer buffer(10);

    auto next_element = 0;
    auto next_received_element = 0;
    for ([[maybe_unused]] auto i : std::views::iota(0, 100'000)) {
        if (dist(gen)) {
            if (buffer.TryPush(next_element)) {
                next_element++;
            }
        } else if (int element = 0; buffer.TryPop(&element)) {
            REQUIRE(next_received_element++ == element);
        }
    }
}

TEST_CASE("Long") {  // NOLINT(readability-function-cognitive-complexity)
    constexpr auto kSize = 100'000;
    constexpr auto kRange = std::views::iota(0, kSize);

    RingBuffer buffer(kSize);
    for (auto x : kRange) {
        REQUIRE(buffer.TryPush(x));
    }
    REQUIRE(buffer.Size() == kSize);

    for (auto i : kRange) {
        int x = 0;
        REQUIRE(buffer.TryPop(&x));
        REQUIRE(x == i);
    }
    REQUIRE(buffer.Empty());
    REQUIRE(buffer.Size() == 0);
}

void OneIterLongFull(RingBuffer& buffer, const size_t size) {
    int x = 0;
    REQUIRE(buffer.TryPop(&x));
    REQUIRE(buffer.TryPush(x));
    REQUIRE_FALSE(buffer.TryPush(x));
    REQUIRE(buffer.Size() == size);
}

TEST_CASE("LongFull") {
    constexpr auto kSize = 100'000;
    constexpr auto kRange = std::views::iota(0, kSize);

    RingBuffer buffer(kSize);
    for (auto x : kRange) {
        REQUIRE(buffer.TryPush(x));
    }

    for ([[maybe_unused]] auto i : std::views::iota(0, 3 * kSize)) {
        OneIterLongFull(buffer, kSize);
    }
}

TEST_CASE("Memory") {
    constexpr auto kSize = 5'000'000;

    RingBuffer buffer(kSize);
    auto ok = true;
    for (auto x : std::views::iota(0, 10 * kSize)) {
        ok &= (buffer.Size() == static_cast<size_t>(std::min(x, kSize)));
        if (buffer.Size() == kSize) {
            int element = 0;
            ok &= buffer.TryPop(&element);
        }
        ok &= buffer.TryPush(x);
    }
    CHECK(ok);
}

/* #ifdef __linux__ */
/* #include "tools/util/util.h" */
/* #include <catch2/catch_message.hpp> */

/* TEST_CASE("Memory Linux") { */
/*     INFO("You have exceeded the 64 MB limit"); */
/*     CHECK(GetMemoryUsage() < 64'000); */
/* } */
/* #endif */
