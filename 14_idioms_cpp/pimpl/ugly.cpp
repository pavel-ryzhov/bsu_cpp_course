...
class Spline::SplineImpl {
    public:
    SplineImpl(std::vector<double>&& x, std::vector<double>&& y, double a, double b) : x_(std::move(x)), y_(std::move(y)) {
...
Spline::Spline(std::vector<double> x, std::vector<double> y, double a, double b) : impl_(std::make_unique<SplineImpl>(std::move(x), std::move(y), a, b)) {}
