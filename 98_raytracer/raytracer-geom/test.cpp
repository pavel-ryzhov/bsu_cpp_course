...
const auto kTestsDir = (GetFileDir(__FILE__, true) / "tests");
...
void CheckCoords(const Triangle& tr, const Vector& point, const std::array<double, 3>& coeffs) {
    ...
    do {  // NOLINT(cppcoreguidelines-avoid-do-while)
        ...
        const Triangle t{tr[i0], tr[i1], tr[i2]};
        const Vector p{point[i0], point[i1], point[i2]};
        const Vector expected = {coeffs.at(i0), coeffs.at(i1), coeffs.at(i2)};
...
TEST_CASE("Triangle") {
    {
        const Triangle triangle{{kX, 0, 0}, {0, kY, 0}, {0, 0, 0}};
    ...
    {
        const Triangle triangle{{1, 0, 0}, {1, kY, 0}, {1, 0, kZ}};
    ...
    {
        const Triangle triangle{{0, 2, 1 + kZ}, {0, 2, 1}, {kX, 2, 1}};
...
TEST_CASE("Intersection1") {
    const Sphere sphere{{0, 0, 0}, 2};
...
TEST_CASE("Intersection2") {
    const Sphere sphere{{0, 0, 0}, 2};
    auto intersection = GetIntersection({{0, 0, 0}, {-1, 0, 0}}, sphere);
    REQUIRE(intersection);
    if (intersection) {
        CheckWithinAbs(intersection->GetPosition(), {-2, 0, 0});
        CheckWithinAbs(intersection->GetNormal(), {1, 0, 0});
    ...
    const Triangle triangle{{0, 0, 0}, {4, 0, 0}, {0, 4, 0}};
...
TEST_CASE("Barycentric coords") {
    ...
    {
        const Triangle t{{3, 0, 0}, {0, 3, 0}, {0, 0, 3}};
    ...
    {
        const Triangle t{{18, 4, 0}, {0, 15, 6}, {6, 0, 24}};
