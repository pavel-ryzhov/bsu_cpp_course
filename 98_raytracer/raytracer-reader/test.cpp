...
TEST_CASE("Scene") {  // NOLINT
...

    {
        ...
        CHECK(obj.GetMaterialPtr()->name == "floor");
...

    {
        ...
        Check(obj.GetInner()[0], 1., 1.59, -1.04);
        Check(obj.GetInner()[1], -1.02, 1.59, -1.04);
        Check(obj.GetInner()[2], -.99, 0., -1.04);
        CHECK(obj.GetMaterialPtr()->name == "backWall");
...

    {
        ...
        Check(obj.GetInner()[0], -1.02, 1.59, -1.04);
        Check(obj.GetInner()[1], -1.02, 1.59, .99);
        Check(obj.GetInner()[2], -1.01, 0., .99);
        CHECK(obj.GetMaterialPtr()->name == "leftWall");
    ...
    for (const auto& object : objects) {
        CHECK(materials_map.contains(object.GetMaterialPtr()->name));
    }
    ...
    REQUIRE(spheres.size() == 2);
    {
        ...
        Check(obj.GetInner().GetCenter(), -.4, .3, -.4);
        CHECK_THAT(obj.GetInner().GetRadius(), WithinAbs(.3));
        CHECK(obj.GetMaterialPtr()->name == "leftSphere");
    }
    {
        const auto& obj = spheres[1];
        Check(obj.GetInner().GetCenter(), .3, .3, 0.);
        CHECK_THAT(obj.GetInner().GetRadius(), WithinAbs(.3));
        CHECK(obj.GetMaterialPtr()->name == "rightSphere");
    }
