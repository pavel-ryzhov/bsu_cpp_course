...
Тесты, как и сам код, можно написать быстро и нечитаемо, а можно аккуратно и надёжно.

### Именование и деление на test suites

Реализация теста на gTest начинается со строчки `TEST(SuiteName, TestName)`. Пока что вам может быть непонятно, какие имена здесь использовать и зачем. Единого правила нет, но предлагаем использовать следующее:

- `SuiteName` один и тот же в рамках одного файла с тестами, и соответствует названию файла с тестами. Например, suite `BigInteger` в файле `test_big_integer.cpp`.
- `SuiteName` отражает компонент проекта, который тестируется. Это может быть как отдельный класс, так и несколько классов, объединённых общей логикой. Например, `BigInteger`, `Matrix`, `SortingAlgorithms` и т.п.
- `TestName` отражает часть функциональности, проверяемую в этом тесте. Например, `Addition`, `WorksWithZeroes`, `EmptyArrays`, `ReadingFromFile` и т.п.

В итоге по названию упавшего теста вы сразу поймёте, где лежит код теста, и что он проверяет.

### Деление на test cases

Вместо написания всех проверок в одном тесте всегда лучше делить их на части, каждая из которых тестирует отдельную функциональность. Во-первых, тогда по результатам тестов можно быстрее понять, что именно не работает. Во-вторых, если тесты работают долго, вы сможете перезапускать только нужные вам, а не все.

При этом, если в нескольких тестах используется одинаковая часть логики, просто вынесите её в функцию.

### Макросы ASSERT_* и EXPECT_*

Все макросы в gTest существуют в двух вариантах: assert и expect. Разница в том, что assert останавливает выполнение теста, если проверка не прошла, а expect — продолжает.

По умолчанию стоит всегда использовать вариант expect, как минимум потому что тогда за один запуск вы сможете увидеть сразу несколько ошибок. Но вариант assert бывает нужен, когда дальнейшая логика теста зависит от успешности проверки:

```cpp
const std::vector<int> result = yourFunction();
