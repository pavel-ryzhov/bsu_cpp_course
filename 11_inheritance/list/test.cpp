...
auto MakeList(Args&&... args) {
    ...
    (list.PushBack(std::forward<Args>(args)), ...);  // NOLINT
    return list;
}
...
TEST_CASE("Empty") {
    const List<int> l1;
    const List<std::string> l2;
    const List<std::vector<int>> l3;
...
TEST_CASE("Back and PushBack") {
    List<int> l;
    l.PushBack(0);
    REQUIRE(l.Back() == 0);  // NOLINT(clang-analyzer-core.UndefinedBinaryOperatorResult)
    l.PushBack(1);
    REQUIRE(l.Back() == 1);
    REQUIRE(l.Size() == 2);
...
struct MoveOnly {
   ...
   private:
    static inline int num_deleted;  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
};

TEST_CASE("Move-only type") {
    REQUIRE(MoveOnly::GetNum() == 0);
    {
        List<MoveOnly> l;
        REQUIRE(l.IsEmpty());
        {
        ...
        REQUIRE(MoveOnly::GetNum() == 1);
        REQUIRE(l.Size() == 1);

        auto l2 = std::move(l);
        REQUIRE(MoveOnly::GetNum() == 1);

        REQUIRE(l.Size() == 0);  // NOLINT
        REQUIRE(l2.Size() == 1);
    }
    REQUIRE(MoveOnly::GetNum() == 2);
}
...
TEST_CASE("Const") {
    ...
    l.PushFront(0);
    l.PushBack(1);
    REQUIRE(l.Front() == 0);  // NOLINT(clang-analyzer-core.UndefinedBinaryOperatorResult)
    REQUIRE(l.Back() == 1);
    const auto& cl = l;
    REQUIRE(cl.Front() == 0);
    REQUIRE(cl.Back() == 1);
...
TEST_CASE("Behaves like a std::list") {
    ...
    std::mt19937 gen{6'022'345};  // NOLINT(cert-msc32-c,cert-msc51-cpp)
    std::uniform_int_distribution random_int{0, 1000};
    std::uniform_int_distribution random_case{0, 6};
...
TEST_CASE("Move") {
    {
        ...
        CheckValid(&l1);     // NOLINT

        auto l3 = MakeList(4, 5);
        l3 = std::move(l2);  // NOLINT
        CheckEquals(l3, std::array{1, 2, 3});
        CheckValid(&l2);     // NOLINT
    }
    {
...
TEST_CASE("Algorithms") {
    {
        ...
        auto result = std::accumulate(list.Begin(), list.End(), 0);
        CHECK(result == 5);
    }
    {
...
TEST_CASE("Fast move insert") {
    constexpr auto kNumIterations = 100'000U;
    constexpr auto kSize = 1'000'000U;
    ...
    REQUIRE(std::ranges::distance(view.begin(), view.end()) == 2UL * kNumIterations);
    for (auto& v : view) {
        REQUIRE(v.empty());
    }
...
TEST_CASE("Big Erase") {
    ...
    std::mt19937 gen{43'539'124};  // NOLINT(cert-msc32-c,cert-msc51-cpp)
