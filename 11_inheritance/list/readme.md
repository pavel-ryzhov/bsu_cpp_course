...
* `void LinkAfter(ListNode* target, ListNode* after)` - _провязывает_ элемент `after` в список после элемента `target`.
...
вопрос: Как представлять пустой список?_. Проще всего завести
фиктивный элемент, чтобы список никогда не был пустым.
...
Также итераторы, указатели и ссылки на элементы списка должны быть не валидными(invalid).
