...
void Check(std::vector<T> data, Predicate pred) {
    ...
    const std::span expected_first_part{copy.begin(), it};
    const std::span expected_second_part{it, copy.end()};
    ...
    const std::span first_part{data.begin(), it};
    const std::span second_part{it, data.end()};
