...
void CheckGroup(const auto& range, const std::vector<std::vector<T>>& expected) {
    ...
    for (const auto& group : Group(range)) {
...
TEST_CASE("Range with step") {
    ...
    CheckRange(4, 4, 1);
    CheckRange(4, 10, 20);
    CheckRange(93'234'123, 341'129'431, 1234);
}

TEST_CASE("Range huge") {
    auto i = 1LL;
    for (const auto& x : Range(1LL, 1UL << 61UL)) {
        REQUIRE(x == i++);
        if (i == 100) {
            break;
        }
...
TEST_CASE("Zip short") {
    ...
    auto it = s.begin();
    int64_t n = 0;
    for (const auto& [c, i] : Zip(s, Range(1UL << 62UL))) {
        REQUIRE(c == *it++);
        REQUIRE(i == n++);
    }
    REQUIRE(it == s.end());
...
TEST_CASE("Zip inf") {
    ...
    auto i = 0LL;
    for (const auto& [i1, i2] : Zip(Range(1UL << 62UL), Range(1UL << 62UL))) {
        if (i1 != i) {
...
struct Int {
    Int(int x) : x_{x} {  // NOLINT(google-explicit-constructor,hicpp-explicit-conversions)
    }
...
TEST_CASE("Group and Zip") {
    const std::list l{1, 1, 2, 2, 3, 3, 3};
    const std::deque d{1, 1, 1, 2, 2, 3, 3};
    const std::vector<std::vector<std::pair<int, int>>> expected = {
