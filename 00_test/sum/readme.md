...
# Из корня проекта.
# собирает нашу цель
bazel build //00_test/sum:public_tests
# собирает и сразу запускает тесты в цели
bazel test //00_test/sum:public_tests
```

Для корректной сборки необходим `g++` версии не ниже 13 или `clang++` версии не ниже 18.0.

Все задачи включены в один проект, поэтому если вы попытаетесь собрать проект целиком, могут возникнуть ошибки. Поэтому при сборке нужно указывать конкретную цель, который вам нужен.
...
bazel test //00_test/sum:public_tests --config=asan
```

Для проверки ошибок clang-tidy
```bash
# Находясь в корне репозитория
bazel build //00_test/sum:public_tests --config=clang-tidy
```

Для проверки ошибок clang-format
```bash
# Находясь в корне репозитория
bazel build //00_test/sum:public_tests --config=clang-format
```
