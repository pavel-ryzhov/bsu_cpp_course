...
Этот класс хранит элементы в кольцевом буфере блоков. В нашей задаче размер блока - 512 байт(128 элементов).
...
* Метод `Capacity`, который будет возвращать размер дека, сколько он использует актуальной памяти.
...
* Используйте `// NOLINT(cppcoreguidelines-owning-memory)` там, где вы выделяете память `new`
